 ...
 */
// Nota: Para usar Supabase, define las variables globales en tu HTML:
// window.SUPABASE_URL = 'tu-url'
// window.SUPABASE_ANON_KEY = 'tu-key'
// window.SUPABASE_CHAT_FUNCTION_URL = 'https://.../functions/v1/chat';
// Alternativamente, descomentar las líneas siguientes y llenar los valores:
// import { createClient } from "@supabase/supabase-js";
// const supabase = createClient('TU_SUPABASE_URL', 'TU_SUPABASE_ANON_KEY');

const SUPABASE_CHAT_FUNCTION_URL = window.SUPABASE_CHAT_FUNCTION_URL || null;
let supabase = null;
...
class IAChat {
  ...
  async handleSendMessage() {
    ...
    // Obtener respuesta de la IA real o fallback local
    const botResponse = await this.getBotResponse(userMessage, isCrisis);
  ...
  /**
   * Obtiene la respuesta del bot: usa un endpoint seguro si está configurado
   */
  async getBotResponse(userMessage, isCrisis) {
    if (isCrisis) {
      return this.generateResponse(userMessage, isCrisis);
    }

    if (!SUPABASE_CHAT_FUNCTION_URL) {
      return this.generateResponse(userMessage, isCrisis);
    }

    try {
      const response = await fetch(SUPABASE_CHAT_FUNCTION_URL, {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) {
        console.warn('No se pudo obtener respuesta de la función:', response.status);
        return this.generateResponse(userMessage, isCrisis);
      }

      const data = await response.json();
      return data.respuesta || this.generateResponse(userMessage, isCrisis);
    } catch (error) {
      console.error('Error llamando a la función de Supabase:', error);
      return this.generateResponse(userMessage, isCrisis);
    }
  }
...
// Función para guardar mensajes en la tabla 'mensajes'
// (Solo funciona si supabase está configurado correctamente)
async function guardarMensaje(usuario, mensaje, respuesta) {
  if (!supabase) {
    console.log("📌 Supabase no configurado. Mensaje no guardado en BD:", { usuario, mensaje, respuesta });
    return;
  }
  ...
  const { data, error } = await supabase
    .from('mensajes')
    .insert([
      { usuario, mensaje, respuesta, fecha: new Date() }
    ])
